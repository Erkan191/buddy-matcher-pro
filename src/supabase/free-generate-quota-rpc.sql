...
as $$
declare
  weekly_limit constant integer := 5;
  emergency_limit constant integer := 1;
  emergency_count_window integer := 0;
...
begin
  ...
  -- Keep daily analytics; only the rolling count controls eligibility.
  select count(*)::integer,
    (count(*) filter (where metadata->>'local_date' = p_quota_date::text))::integer
    into emergency_count_window, emergency_count_today
  from public.usage_events
  where event_type = 'free_generate_success'
    and created_at > window_start
  ...
  emergency_remaining := greatest(0, emergency_limit - emergency_count_window);
  ...
  if mode_to_record = 'weekly' then
    ...
    emergency_count_window := emergency_count_window + 1;
    emergency_remaining := greatest(0, emergency_limit - emergency_count_window);
  end if;
  ...
  next_metadata := coalesce(p_metadata, '{}'::jsonb) || jsonb_build_object(
    ...
    'emergency_count_window', emergency_count_window,
